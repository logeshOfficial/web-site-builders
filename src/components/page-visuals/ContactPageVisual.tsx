...
const CHANNELS = [
  {
    ...
    icon: (
      <svg className="h-5 w-5 text-teal-400" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        <path
          strokeLinecap="round"
  ...
  return (
    <PageHeroVisual caption={v.caption}>
      <div className="relative h-[220px] w-full sm:h-[240px]">
        <div className="absolute left-1/2 top-1/2 flex h-20 w-20 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-teal-400/20 bg-teal-500/10">
        ...
        {CHANNELS.map(({ key, position, icon }) => (
          <div
            key={key}
            className={`absolute z-10 flex flex-col items-center gap-1 rounded-xl border border-slate-600/60 bg-slate-800/90 px-3 py-2.5 shadow-lg ${position}`}
          >
            {icon}
