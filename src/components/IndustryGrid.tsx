      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-end">
          <div className="max-w-2xl">
            <h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">{t.industries.title}</h2>
            <p className="mt-4 text-lg text-slate-600">{t.industries.description}</p>
          </div>
          <Link href="/contact" className="text-sm font-semibold text-teal-700 hover:text-teal-800">
            {t.industries.contactLink}
          </Link>
        </div>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {t.industries.list.map((industry) => (
            ...
            >
              <h3 className="text-xl font-semibold text-slate-900 group-hover:text-teal-700">{industry.title}</h3>
        ...
        <p className="mt-8 text-center text-sm text-slate-500">{t.industries.alsoServing}</p>
      </div>
    </section>
  );
}
